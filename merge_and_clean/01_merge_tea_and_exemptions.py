...
data_district = clean_for_merge.merge_district_and_exemptions(tea_df = tea_district, laws_df = laws, geo_df= geo)
...
# %%
# School-Level
data_school = clean_for_merge.merge_school_and_exemptions(tea_df = tea_school, laws_df= laws, teacher_df = teachers, geo_df = geo)
...
gdid_school['doi_year'] = np.where((gdid_school.doi_year == 2015), np.nan, gdid_school.doi_year) #drop first implementer (one district)
...
subject_grade = reshape.merge(gdid_school, left_on = ['campus', 'year'], right_on = ['campus', 'year'])
