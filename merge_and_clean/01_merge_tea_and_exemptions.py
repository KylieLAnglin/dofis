...
def merge_district_and_exemptions(tea_df = tea_district, laws_df = laws):
...
def merge_school_and_exemptions(tea_df = tea_school, laws_df = laws):
    tea, laws = resolve_merge_errors(tea_df, laws_df)
    data = tea.merge(laws, left_on='distname', right_on='distname', how='left', indicator=True)
    data.loc[(data['_merge'] == 'both'), 'doi'] = True
    data.loc[(data['_merge'] == 'left_only'), 'doi'] = False
...
tea_district = import_tea_district()
tea_school = import_tea_school()
laws = import_laws()
geo = import_geo()
