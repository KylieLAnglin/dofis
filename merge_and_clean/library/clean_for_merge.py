...
def replace_column_values(df, col_name, string_to_replace,
                          string_to_replace_with):
    """Run string replace on all values in a column"""
    df = df.copy()
    df[col_name] = df[col_name].str.replace(
        string_to_replace, string_to_replace_with).str.strip()
    return df
...
def get_not_in(df_a, col_a, df_b, col_b):
    a_not_in_b = df_a[[elem not in list(df_b[col_b])
                       for elem in list(df_a[col_a])]]
    return a_not_in_b
...
def merge_district_and_exemptions(tea_df, laws_df, geo_df):
    ...
    data = tea.merge(laws, left_on='distname', right_on='distname',
                     how='left', indicator=True,
                     validate='m:1')
    data.loc[(data['_merge'] == 'both'), 'doi'] = True
    data.loc[(data['_merge'] == 'left_only'), 'doi'] = False
    data = data.merge(geo_df, left_on='cntyname', right_on='county',
...
def import_tea_school():
    tea = pd.read_csv(os.path.join(start.data_path, 'tea', 'desc_c_long.csv'),
                      sep=",", low_memory=False)
    variables = ['year', 'campus', 'campname', 'campischarter', 
