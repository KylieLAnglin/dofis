...
def fix_parser_error(input_path):
    temp_directory = os.path.join(data_path, 'temp')
...
def clean_dref(year):
    if year == 'yr1112':
        ...
        filename = 'DREF.dat'
    dref = pd.read_csv(os.path.join(data_path, 'dref', year, filename), sep=",")
...
def clean_ddem(year):
    ...
    if year == 'yr1718':
        year = 'yr1617'
    if year == 'yr1112':
        ddem1 = pd.read_csv(os.path.join(data_path, 'ddem', year, 'dstud.csv'), sep=",")
        ddem2 = pd.read_csv(os.path.join(data_path, 'ddem', year, 'dstaf.csv'), sep=",")
        ...
        ddem = pd.read_csv(os.path.join(data_path, 'ddem', year, filename), sep=",")
        ddem_tokeep = {
            'DISTRICT': 'district',
            'DPSATOFC': 'teachers_num',
            'DPST00FC': 'teachers_new_num',
...
def clean_scores(year, subject):
    ...
    try:
        dscores = pd.read_csv(os.path.join(data_path, 'dscores', subject, file), sep=",")
    except:
        new_path = fix_parser_error(os.path.join(data_path, 'dscores', subject, file))
