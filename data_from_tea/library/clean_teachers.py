...
def split_grades(df: pd.DataFrame, col: str, low_name: str, high_name: str):
    ...
    df[low_name], df[high_name] = df[col].str.split("-").str
...
def gen_subject(
    ...
    """Generate binary variable for whether a certification
    is in a particular TEA certification SUBJECT_AREA.

    Args:
        df (pd.DataFrame): data set containing SUBJECT_AREA column
        new_col (str): New column name
        area_tuple (tuple): tuple with TEA cert area column and value
        level_tuple (tuple, optional): tuple with TEA cert level column and value. Defaults to {}.
        standard_tuple (tuple, optional): limit trues to standard certifications? tuple with standard cert bool col and value. Defaults to {}.
    """

    df[new_col] = np.where(df[area_tuple[0]] == area_tuple[1], True, False)

    if len(level_tuple) > 0:
        df[new_col] = np.where(df[level_tuple[0]] == level_tuple[1], df[new_col], False)

    if len(standard_tuple) > 0:
        df[new_col] = np.where(
            df[standard_tuple[0]] == standard_tuple[1], df[new_col], False
        )
