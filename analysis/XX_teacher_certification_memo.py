...
cert_df = pd.DataFrame(exempt_cert).groupby(["campus"]).agg({"doi_year": "mean"})
...
plt.plot(cert_counts.index, cert_counts.doi_year.cumsum(), color="gray")
...
event_study_model = (
    "certified ~ + 1 + pre5 + pre4 + pre3 + pre2 + "
...
coef_df = pd.DataFrame(
    {"coef": nonparametric, "err": nonparametric_se, "year": [-5, -4, -3, -2, -1, 1, 2]}
)
...
_ = ax.set_xticklabels(
    ["Pre5", "Pre4", "Pre3", "Pre2", "Pre1", "Post1", "Post2"], rotation=0
)
