...
for reg, col in zip(['reg25_0811', 'reg25_081', 'reg25_0812', 'reg25_082',
                     'reg25_112', 'reg25_113', 'reg21_003', 'reg21_053',
                     'reg21_057',
                     'reg21_102', 'reg21_401', 'reg21_352', 'reg25_092',
                     'reg37_0012', 'reg25_036'],
                    list(range(2, 17))):
    ...
    for df, row in zip(dfs, rows):
        tables.just_diff_to_excel(file=start.table_path +
                                  'table4_balance_exemptions.xlsx',
                                  df=df,
                                  diff_col='Difference',
                                  se_col='Std. Error', pvalue_col='P-value',
                                  start_col=col, start_row=row)
    tables.n_to_excel(file=start.table_path + 'table4_balance_exemptions.xlsx',
                      col=col, row=5, n=data[data[reg] == 1].district.nunique())
