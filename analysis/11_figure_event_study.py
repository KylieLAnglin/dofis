...
math_agg = pd.read_excel(start.TABLE_PATH + "results_math_ag_raw.xlsx")
reading_agg = pd.read_excel(start.TABLE_PATH + "results_reading_ag_raw.xlsx")
...
ax1.scatter(
    x=pd.np.arange(math_df.shape[0]),
    marker="s",
    s=120,
    y=math_df["coef"],
    color=colors,
...
for pos, y, err, color in zip(
    pd.np.arange(math_df.shape[0]), math_df["coef"], math_df["errsig"], colors
):
...
for pos, y, err, color in zip(
    pd.np.arange(reading_df.shape[0]), reading_df["coef"], reading_df["errsig"], colors
):
