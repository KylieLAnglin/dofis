...
for subgroup in subgroups:
    results["math_yr15std"][subgroup]["df"] = pd.read_excel(
        start.TABLE_PATH + "results_math_yr15std_" + subgroup + "_ag_raw_weights.xlsx"
    )
    ...
    results["reading_yr15std"][subgroup]["df"] = pd.read_excel(
        start.TABLE_PATH
        + "results_reading_yr15std_"
        + subgroup
        + "_ag_raw_weights.xlsx"
    )
...
fig.savefig(
    start.TABLE_PATH + "formatted_results/AppendixD_FigureD5.pdf", bbox_inches="tight"
)
