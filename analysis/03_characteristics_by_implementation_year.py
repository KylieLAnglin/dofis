...
sys.path.append("../")
from library import analysis, characteristics, regulations, tables, start
...
tables.n_to_excel(file=start.table_path + table_file,
                  col=2, row=31, n=len(data[data.distischarter == 0]))
tables.n_to_excel(file=start.table_path + table_file,
                  col=3, row=31, n=len(data[data.doi_year == col_names[1]]))
tables.n_to_excel(file=start.table_path + table_file,
                  col=4, row=31, n=len(data[data.doi_year == col_names[2]]))
tables.n_to_excel(file=start.table_path + table_file,
                  col=5, row=31, n=len(data[data.doi_year == col_names[3]]))
tables.n_to_excel(file=start.table_path + table_file,
                  col=6, row=31,
