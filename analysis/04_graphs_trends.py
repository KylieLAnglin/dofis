...
def create_group_df(df):
    new_df = pd.DataFrame(df.groupby(['year']).agg(
        {'avescores': ['mean', 'sem']}))
    new_df = new_df.rename(columns={'mean': 'score_mean', 'sem': 'score_se'})
    new_df['ub'] = new_df['avescores']['score_mean'] + \
        new_df['avescores']['score_se']
    new_df['lb'] = new_df['avescores']['score_mean'] - \
        new_df['avescores']['score_se']
    return new_df
...
plt.plot(list(df_treat2017[df_treat2017.index <= 2017].index),
         ...
         color='green', label='2016-17 DOI Implementers')
plt.plot(list(df_treat2018[df_treat2018.index <= 2018].index),
         df_treat2018[df_treat2018.index <= 2018]['avescores']['score_mean'],
         color='blue', label='2017-18 DOI Implementers')
...
plt.fill_between(list(df_treat2017[df_treat2017.index <= 2017].index),
                 df_treat2017[df_treat2017.index <= 2017].lb,
                 df_treat2017[df_treat2017.index <= 2017].ub,
                 color='green', alpha=.2)
plt.fill_between(list(df_treat2018[df_treat2018.index <= 2018].index),
                 df_treat2018[df_treat2018.index <= 2018].lb,
                 df_treat2018[df_treat2018.index <= 2018].ub,
                 color='blue', alpha=.2)
plt.fill_between(list(df_treat2019[df_treat2019.index <= 2019].index),
...
plt.savefig(start.table_path + 'parallel_trends_by_adoption.png',
            bbox_inches="tight")
...
def create_group_df(df, outcome):
    ...
    new_df = pd.DataFrame(df.groupby(['year']).agg(
        {'outcome': ['mean', 'sem']}))
    new_df = new_df.rename(columns={'mean': 'score_mean', 'sem': 'score_se'})
    new_df['ub'] = new_df['outcome']['score_mean'] + \
        new_df['outcome']['score_se']
    new_df['lb'] = new_df['outcome']['score_mean'] - \
        new_df['outcome']['score_se']
    return new_df
...
title_labels = {'avescores': 'Average STAR',
                ...
                'biology': 'Biology', 'algebra': 'Algebra',
                'eng1': 'English I',
...
for outcome in title_labels:
    df_treat2017 = create_group_df(
        data[data.doi_year == 2017], outcome=outcome)
    df_treat2018 = create_group_df(
        data[data.doi_year == 2018], outcome=outcome)
    df_treat2019 = create_group_df(
    ...
    ax.plot(list(df_treat2017.index),
            df_treat2017['outcome']['score_mean'],
            label='2016-17 DOI Implementers')
    ax.plot(list(df_treat2018.index),
            df_treat2018['outcome']['score_mean'],
            label='2017-18 DOI Implementers')
    ax.plot(list(df_treat2019.index),
            df_treat2019['outcome']['score_mean'],
            label='2018-19 DOI Implementers')

    ax.legend()
    ax.fill_between(list(df_treat2017.index), df_treat2017.lb,
                    df_treat2017.ub, alpha=.2)
    ax.fill_between(list(df_treat2018.index), df_treat2018.lb,
                    df_treat2018.ub, alpha=.2)
    ax.fill_between(list(df_treat2019.index), df_treat2019.lb,
                    df_treat2019.ub, alpha=.2)
